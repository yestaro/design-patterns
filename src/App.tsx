    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900 text-left text-sm md:text-base lg:text-lg">
      ...
      <div className="max-w-full mx-auto">
        <header className="mb-6 text-center">
          ...
          <p className="text-slate-500 text-sm lg:text-base font-medium text-center italic mt-1 font-bold text-blue-600">從 if - else 到抽象、封裝與介面，以架構思維實踐高品質程式碼</p>
        </header>
        ...
        <div className="flex flex-wrap gap-1 mb-6 p-1 bg-white rounded-xl shadow-sm border border-slate-200">
          {[
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-bold transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'}`}
            >
              <tab.i size={20} /> {tab.l}
            </button>
          ))}
