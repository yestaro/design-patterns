                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                ...
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>子類別差異極小、絕大部分屬性共用？</b> 👉 選 <button onClick={() => setActiveInheritance('sti')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">STI (Single Table Inheritance)</button>，雖然會有 NULL，但查詢最快、ORM 支援最好。</li>
                                    <li><b>子類別屬性差異大，重視資料庫層級的完整性約束設計，且查詢通常「跨越多個階層」？</b> 👉 選 <button onClick={() => setActiveInheritance('cti')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">CTI (Class Table Inheritance)</button>，最正規劃，但需忍受 JOIN 效能損耗。</li>
                                    <li><b>子類別屬性差異巨大，且經常需要「獨立查詢」特定子類別？</b> 👉 選 <button onClick={() => setActiveInheritance('concrete')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Concrete Table Inheritance</button>，物理隔離，零 JOIN 成本。</li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                                ...
                                <div className="mt-6 bg-slate-900 rounded-xl overflow-hidden shadow-inner border border-slate-800">
                                    <div className="bg-slate-800 px-4 py-2 border-b border-slate-700 flex flex-col gap-1">
                                        <div className="flex items-center gap-2">
                                            <span className="text-indigo-400 font-black text-sm">SQL 範例</span>
                                            <span className="text-slate-400 text-xs">查詢「圖片」與「文件」檔案 (多重 LEFT JOIN)</span>
                                        </div>
                                    </div>
                                    <CodeBlock
                                        language="sql"
                        ...
                        {activeInheritance === 'concrete' && (
                            ...
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>物理隔離：</b> 每張表完全獨立存儲父類與子類欄位。雖然 `JOIN` 為零，但資料完整性與共用欄位的維護（如 Size 統計）極為困難，若要跨類別查詢，須 UNION ALL，效能會大幅下降。</p>

                                <div className="mt-6 bg-slate-900 rounded-xl overflow-hidden shadow-inner border border-slate-800">
                                    <div className="bg-slate-800 px-4 py-2 border-b border-slate-700 flex items-center gap-2">
                                        <span className="text-indigo-400 font-black text-sm">SQL 範例</span>
                                        <span className="text-slate-400 text-xs">建立全域檔案清單 (大量 UNION ALL)</span>
                                    </div>
                                    <CodeBlock
                                        language="sql"
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                <p className="font-bold text-indigo-800 mb-2 flex items-center gap-2">💡 情境決策指南</p>
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>階層很淺（通常 2-3 層），主要只查詢「直屬子節點」？</b> 👉 選 <button onClick={() => setActiveHierarchy('adjacency')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Adjacency List</button>，結構最直覺，寫入與移動節點無負擔。</li>
                                    <li><b>需要頻繁且快速地撈取「某個節點底下的所有子子孫孫」（如麵包屑導覽）？</b> 👉 選 <button onClick={() => setActiveHierarchy('path')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Path Enumeration</button>，字串 LIKE 搜尋極快，但移動節點成本高。</li>
                                    <li><b>階層極深、需要複雜的祖先/子孫關聯查詢，且必須保證 Referential Integrity？</b> 👉 選 <button onClick={() => setActiveHierarchy('closure')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Closure Table</button>，用空間換取最靈活的查詢時間。</li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        {activeHierarchy === 'closure' && (
                            ...
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>空間換時間：</b> 最正規、查詢最強大的層級設計。透過一張專門的關聯表紀錄所有「祖先-子代」的笛卡兒積。可應付極度複雜的層級、多重隸屬關係，需要頻繁查詢「任意兩節點關係」的企業級應用。</p>
                            </div>
                        )}
                    </div>
                </section>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                <p className="font-bold text-indigo-800 mb-2 flex items-center gap-2">💡 情境決策指南</p>
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>關聯對象可能無限擴充，且堅持最高標準的資料庫正規劃與 Referential Integrity？</b> 👉 選 <button onClick={() => setActivePolymorphic('reverse')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Reverse Polymorphic</button>，建立專屬中介表，解耦最徹底。</li>
                                    <li><b>使用 Rails/Laravel 等 ORM 開發，且極度看重程式碼便利性？</b> 👉 選 <button onClick={() => setActivePolymorphic('belongsTo')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Polymorphic Belongs-To</button>，直接使用框架內建支援，開發最快。</li>
                                    <li><b>未來要關聯的實體數量確定且很少（就固定 2-3 個），且不允許資料髒亂？</b> 👉 選 <button onClick={() => setActivePolymorphic('exclusive')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Exclusive Foreign Keys</button>，完全確保關聯實體存在。</li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {activePolymorphic === 'reverse' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium mt-6"><b>高度解耦：</b> 符合正規化精神且保留穩固的 FK 約束。如果是 <b>Concrete Table Inheritance</b> 的設計，未來擴充實體時，只需新增專屬的關聯表（如 `VideoTags`），完全無需異動既有資料表。</p>
                            </div>
                        )}
                        {activePolymorphic === 'belongsTo' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                ...
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                    <p className="text-sm text-slate-600 leading-relaxed"><b>情境：</b> 建立一張「通用」的 `ItemTags` 關聯表。使用 `ItemType` 記錄目標表格名稱 (Word 還是 Image，甚至將來不是檔案的Tag，而是針對其它表的Tag)，`ItemID` 記錄它的 ID。</p>
                                </div>
                                <div className="sample-table-container shadow-lg shadow-slate-100/50">
                                    <table className="sample-table">
                                        <thead>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                <p className="font-bold text-indigo-800 mb-2 flex items-center gap-2">💡 情境決策指南</p>
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>需要把動態屬性當作頻繁的 SQL 搜尋條件（例如「找出所有螢幕尺寸包含 1080p」的商品）？</b> 👉 考慮 <button onClick={() => setActiveMetadata('eav')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">EAV</button>，方便建立索引。</li>
                                    <li><b>動態屬性只是純撈取展示，幾乎不作為 JOIN 或複雜 WHERE 條件？</b> 👉 秒選 <button onClick={() => setActiveMetadata('json')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Hybrid JSON</button>，單表單欄位輕鬆搞定。</li>
                                    <li><b>系統是老舊框架，不支援 JSON，DBA 也不准開新表（或開表流程極繁瑣）？</b> 👉 考慮 <button onClick={() => setActiveMetadata('overload')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Multi-Column Overload</button> 作為過渡妥協方案。</li>
                                </ul>
                            </div>
                        </div>
                ...
                <section className="pt-12 border-t border-slate-100 space-y-8 pb-12">
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                <p className="font-bold text-indigo-800 mb-2 flex items-center gap-2">💡 情境決策指南</p>
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>最標準的作法，查詢線上資料時絕對不想被歷史紀錄干擾？</b> 👉 選 <button onClick={() => setActiveVersioning('shadow')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Shadow Table (影子表)</button>，效能與架構最乾淨。</li>
                                    <li><b>需要頻繁比較不同版本，或是同一個實體允許同時存在多個「草稿」分支？</b> 👉 選 <button onClick={() => setActiveVersioning('inline')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Inline Versioning (行內版本)</button>。</li>
                                    <li><b>極度重視「操作歷程」，需要紀錄誰在什麼時候修改了哪裡的字（甚至需要復原到精確秒數）？</b> 👉 選 <button onClick={() => setActiveVersioning('event')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Event Sourcing (事件溯源)</button>。</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-3 w-full">
                            ...
                            >
                                <span className={`text-sm font-black tracking-wide ${activeVersioning === 'shadow' ? 'text-indigo-700' : 'text-slate-500'}`}>Shadow Table (影子表)</span>
                                <div className="flex gap-2.5 mt-1.5 text-xs items-center font-bold">
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Query:</span><span className="text-amber-500">5/5</span></div>
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Integrity:</span><span className="text-amber-500">4/5</span></div>
                            ...
                            >
                                <span className={`text-sm font-black tracking-wide ${activeVersioning === 'inline' ? 'text-indigo-700' : 'text-slate-500'}`}>Row Versioning (行內版本)</span>
                                <div className="flex gap-2.5 mt-1.5 text-xs items-center font-bold">
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Query:</span><span className="text-rose-500">2/5</span></div>
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Integrity:</span><span className="text-amber-500">3/5</span></div>
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Traceability:</span><span className="text-amber-500">4/5</span></div>
                            ...
                            >
                                <span className={`text-sm font-black tracking-wide ${activeVersioning === 'event' ? 'text-indigo-700' : 'text-slate-500'}`}>Event Sourcing (事件溯源)</span>
                                <div className="flex gap-2.5 mt-1.5 text-xs items-center font-bold">
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Query:</span><span className="text-rose-500">1/5</span></div>
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Integrity:</span><span className="text-amber-500">5/5</span></div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {activeVersioning === 'shadow' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                <div className="flex flex-col gap-2">
                                    <h5 className="text-lg font-black text-slate-700 italic">Shadow Table (影子歷史表) - 主從分離的最乾淨設計</h5>
                                </div>
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                    <p className="text-sm text-slate-600 leading-relaxed"><b>情境：</b> 主表 <code className="bg-white px-1 py-0.5 rounded border border-slate-200">Entries</code> 只保存「最新/正式」狀態。任何時候 UPDATE 或 DELETE 主表，就觸發邏輯（程式碼或 DB Trigger），把舊資料複製一份存入結構完全一樣的 <code className="bg-white px-1 py-0.5 rounded border border-slate-200">Entries_History</code> 表裡。</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                                    <div className="sample-table-container shadow-lg shadow-slate-200/50">
                                        <div className="bg-slate-800 text-white px-4 py-1 font-black text-sm uppercase tracking-widest text-center italic">主表：Entries (永遠最新)</div>
                                        <table className="sample-table">
                                            <thead>
                                    ...
                                    <div className="sample-table-container shadow-lg shadow-amber-100/50">
                                        <div className="bg-amber-600 text-white px-4 py-1 font-black text-sm uppercase tracking-widest text-center italic">歷史表：Entries_History (追加寫入)</div>
                                        <table className="sample-table">
                                            <thead>
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>極速查詢：</b> 全域搜尋或 JOIN 主表時，效能不受成千上萬筆歷史紀錄的拖累（因為它們被物理隔離了）。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>關鍵細節：</b> <code className="text-rose-500 font-bold">歷史表的 EntryID 絕對不能建立 Foreign Key 約束</code>！就如同表中的 <code className="font-bold text-red-500">404</code>，如果主表資料被刪除了，而這裡又有強 FK 綁定，資料庫就會報錯阻止刪除（或引發 CASCADE 災難把珍貴的歷程全砍了）。所以這理只能是個「軟關聯」的數字。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>架構負擔：</b> 需維護兩張 Schema 相同的表。每次主表加減欄位（如新增 `Author`），影子表也必須跟著 ALTER TABLE。</p>
                            </div>
                        )}
                        {activeVersioning === 'inline' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                <div className="flex flex-col gap-2">
                                    <h5 className="text-lg font-black text-slate-700 italic">Row Versioning (行內版本) - 狀態維度合併</h5>
                                </div>
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                    <p className="text-sm text-slate-600 leading-relaxed"><b>情境：</b> 不開新表，所有的新舊資料全部混在一起存。取消原本把 PK 當作唯一識別的方法，改以一個代表號（如 `DocumentID`），結合一個 `Version` 或 `IsCurrent` 的旗標來設計 Schema。</p>
                                </div>
                                <div className="sample-table-container shadow-lg shadow-indigo-100/50">
                                    <table className="sample-table">
                                        <thead>
                                            <tr><th className="text-red-500">EntryID (PK)</th><th className="text-purple-600">DocID (文件群組)</th><th className="text-emerald-600 font-black">IsCurrent</th><th className="text-emerald-600 font-black">Version</th><th>Content</th></tr>
                                        </thead>
                                        <tbody className="text-slate-700">
                                            <tr className="text-slate-400 opacity-70"><td className="font-bold text-red-500">1</td><td className="font-bold text-purple-600">101</td><td className="font-bold text-slate-400">FALSE</td><td className="font-bold">1.0</td><td>文件初稿...</td></tr>
                                            <tr className="bg-emerald-50/50"><td className="font-bold text-red-500">2</td><td className="font-bold text-purple-600">101</td><td className="font-black text-emerald-600">TRUE</td><td className="font-black text-emerald-600">2.0</td><td className="text-slate-800">最新修改的版本...</td></tr>
                                            <tr className="text-slate-400 opacity-70"><td className="font-bold text-red-500">3</td><td className="font-bold text-purple-600">101</td><td className="font-bold text-slate-400">FALSE</td><td className="font-bold">2.1(Draft)</td><td>某人寫一半的草稿...</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>彈性極高：</b> 由於允許同一個群組（DocID）下有多種不同狀態的紀錄，因此極度適合實作「多人協作編輯」、「送審草稿與正式上線並存」這種複雜的業務邏輯。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>代價慘痛：</b> 所有的 SQL 查詢，所有的 JOIN 條件，<b>必定</b>都要強制補上 <code className="text-rose-500 font-bold">WHERE IsCurrent = TRUE</code>，只要開發者一不小心漏寫，整個系統就會暴亂。另外，當一個系統十年的歷史積累後，這張表會變得無比巨大且緩慢。</p>
                            </div>
                        )}
                        {activeVersioning === 'event' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                <div className="flex flex-col gap-2">
                                    <h5 className="text-lg font-black text-slate-700 italic">Event Sourcing (事件溯源) - 回放過去的黑盒子</h5>
                                </div>
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                    <p className="text-sm text-slate-600 leading-relaxed"><b>情境：</b> 根本不存「最終的內容」，只存「你對文件做了什麼事 (Delta)」。就跟 Git 的 Commit 一樣，當要讀取文件的最新狀態時，就是把所有的事件從頭「回放 (Replay)」一遍計算出來的結果。</p>
                                </div>
                                <div className="sample-table-container shadow-lg shadow-amber-100/50">
                                    <table className="sample-table">
                                        <thead>
                                            <tr><th>EventID</th><th className="text-red-500">EntryID (目標)</th><th className="text-indigo-600 font-black">EventType</th><th className="text-slate-500">Payload (JSON)</th><th>CreatedAt</th></tr>
                                        </thead>
                                        <tbody className="text-slate-700 text-sm">
                                            <tr><td>1</td><td className="font-bold text-red-500">303</td><td className="text-indigo-600 font-bold uppercase">文件建立</td><td className="font-mono text-xs">{"{ \"Name\": \"草稿\" }"}</td><td>10:00:00</td></tr>
                                            <tr className="bg-slate-50"><td>2</td><td className="font-bold text-red-500">303</td><td className="text-indigo-600 font-bold uppercase">變更名稱</td><td className="font-mono text-xs">{"{ \"Old\": \"草稿\", \"New\": \"企劃\" }"}</td><td>10:05:30</td></tr>
                                            <tr><td>3</td><td className="font-bold text-red-500">303</td><td className="text-emerald-600 font-bold uppercase">內容加上</td><td className="font-mono text-xs">{"{ \"Diff\": \"+新增了一行段落\" }"}</td><td>10:15:10</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>上帝視角：</b> 系統擁有真正的「時光機」。你可以隨時重建出 10:05:31 時文件的確切模樣，沒有任何資訊被永久覆蓋。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>開發難度：</b> 極高。在關聯式資料庫中直接進行這類實作會導致簡單的搜尋變得如同地獄（你無法直接 <code className="text-rose-500 font-bold">WHERE Name = '企劃'</code>，因為 Name 藏在事件中）。實務中通常會搭配 CQRS，外加一個 Materialized View 來專門應付讀取。</p>
                            </div>
                        )}
                    </div>
                </section>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 text-sm text-slate-600 leading-relaxed ml-11">
                                <p className="font-bold text-indigo-800 mb-2 flex items-center gap-2">💡 情境決策指南</p>
                                <ul className="list-disc pl-5 space-y-1.5">
                                    <li><b>最標準的專案防禦配置，不用想太多加就對了？</b> 👉 <button onClick={() => setActiveAudit('embedded')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Embedded (直接加在原表)</button>，也就是經典的 CreatedAt/UpdatedAt 組合，軟刪除 (Soft Delete) 也是這個範疇。</li>
                                    <li><b>法規嚴格要求（如醫療或金融系統），需查核任何一張表「誰改了什麼欄位的哪個字」？</b> 👉 <button onClick={() => setActiveAudit('centralized')} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none transition-colors">Centralized Audit Log (集中稽核表)</button>，不污染原結構，單一中心化監控。</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-3 w-full">
                            ...
                            >
                                <span className={`text-sm font-black tracking-wide ${activeAudit === 'embedded' ? 'text-indigo-700' : 'text-slate-500'}`}>Embedded Columns (嵌入共用欄)</span>
                                <div className="flex gap-2.5 mt-1.5 text-xs items-center font-bold">
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Query:</span><span className="text-amber-500">5/5</span></div>
                            ...
                            >
                                <span className={`text-sm font-black tracking-wide ${activeAudit === 'centralized' ? 'text-indigo-700' : 'text-slate-500'}`}>Centralized Log (集中稽核日誌)</span>
                                <div className="flex gap-2.5 mt-1.5 text-xs items-center font-bold">
                                    <div className="flex items-center gap-1"><span className="text-slate-400">Query:</span><span className="text-rose-500">2/5</span></div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {activeAudit === 'embedded' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>極佳的實用性：</b> 這涵蓋了 90% 查核「誰建了這筆資料、最後誰改的」的需求。加入 <code className="text-rose-500 font-bold">DeletedAt</code> 即為業界俗稱的「軟刪除 (Soft Delete)」，不真實刪除資料，方便救援並保留關聯。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>弱點：</b> 這個做法是<b>覆寫</b>的。如果這筆紀錄被更新了 100 次，你在這裡也只會看見這「最後一次 (第100次)」是誰做的。中間 99 次的更新歷史都會永遠消失。</p>
                            </div>
                        )}
                        {activeAudit === 'centralized' && (
                            <div className="group space-y-4 border-l-4 border-slate-200 pl-6 py-2">
                                <div className="flex flex-col gap-2">
                                    <h5 className="text-lg font-black text-slate-700 italic">Centralized Audit Log - Polymorphic 的應用</h5>
                                </div>
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                                    <p className="text-sm text-slate-600 leading-relaxed"><b>情境：</b> 精準紀錄「原值與新值的變化」。我們可以使用 **Polymorphic Belongs-To** 設計一張通用的 `System_AuditLogs` 巨集表。無論是訂單修改、會員資料異動，全都在這張表來集中管理。</p>
                                </div>
                                <div className="sample-table-container shadow-lg shadow-amber-100/50">
                                ...
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>解耦與擴充：</b> 對於原業務表格來說是零污染。且這張表可以無縫拋轉給 Elasticsearch 或是資料倉儲（Data Warehouse）去專門應付合規查詢。</p>
                                <p className="text-sm text-slate-500 leading-relaxed font-medium"><b>難以關聯查詢：</b> 這是一張混雜的多型表，加上數值被塞在 JSON 裡，你幾乎不可能下出 <code className="text-rose-500 font-bold">SQL: 將 AuditLogs JOIN 至關聯的訂單並查出某商品被誰修改</code>。它只適合「以特定 ID 為入口向下單向呈現」。</p>
                            </div>
                        )}
                    </div>
                </section>
