...
| **匯出骨架**   | **Template Method (模板方法模式)** | 封裝演算法骨架（遞迴走訪、層級縮排），將具體實作交給子類別（如 XML、Markdown 格式匯出）。             |
...
| **介面轉換**   | **Adapter (轉接器模式)**          | 實作 `IObserver` 介面，當接收到系統事件時（如掃描進度），即時將資料適配轉接為 UI 期待的格式，推播更新 Dashboard。 |
| **檔案操作**   | **Command (命令模式)**            | 將刪除、移動（拖曳）、複製貼上等操作封裝為物件，支援操作的排程、與**Undo/Redo** 功能。              |
| **演算法切換** | **Strategy (策略模式)**           | 動態切換不同的排序或搜尋演算法。                                                                        |
| **剪貼簿**     | **Singleton (單例模式)**          | 確保全系統只有一個剪貼簿實例，統一管理複製與貼上的資料。                                                |
| **資源共享**   | **Flyweight + Factory (享元模式)**  | 透過標籤工廠共享相同配色的標籤實體，達成一致化，並減少記憶體開銷。                                      |
| **標籤互動**   | **Mediator (中介者模式)**         | 處理檔案與標籤之間的複雜互動關係，降低物件間的耦合度。                                                  |
| **系統介面**   | **Facade (外觀模式)**             | 提供一個簡單的統一介面 (`FileSystemFacade`)，隱藏背後複雜的子功能（Composite, Command, Visitor 等）。 |
